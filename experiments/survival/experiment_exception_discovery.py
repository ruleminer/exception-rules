...
from lifelines.utils import ConvergenceError, restricted_mean_survival_time
    ...
    def _evaluate_rules(self, ruleset, dataset, X, y):
        ...
        for rule in rules:
            if rule.exception_rule is not None:
                ...
                er_km = KaplanMeierEstimator().fit(survival_time[er_covered], survival_status[er_covered], update_additional_informations=False)
                cr_km = KaplanMeierEstimator().fit(survival_time[cr_covered], survival_status[cr_covered], update_additional_informations=False)
                rr_km = KaplanMeierEstimator().fit(survival_time[rr_covered], survival_status[rr_covered], update_additional_informations=False)
 
 
                cr_er_stats_and_pvalue = KaplanMeierEstimator().compare_estimators(
                            cr_km, er_km)
               
                rr_er_stats_and_pvalue = KaplanMeierEstimator().compare_estimators(
                        rr_km, er_km)
 
                   
                cr_rr_stats_and_pvalue = KaplanMeierEstimator().compare_estimators(
                        cr_km, rr_km)
               
                rules_dict["CR vs ER p_value"] = cr_er_stats_and_pvalue["p_value"]
                rules_dict["RR vs ER p_value"] = rr_er_stats_and_pvalue["p_value"]
                rules_dict["CR vs RR p_value"] = cr_rr_stats_and_pvalue["p_value"]
                ...
                tau = np.min([np.max(survival_time_cr), np.max(survival_time_er), np.max(survival_time_rr)])
                ...
                rmst_cr = restricted_mean_survival_time(km_cr, t=tau, return_variance=False)
                rmst_er = restricted_mean_survival_time(km_er, t=tau, return_variance=False)
                rmst_rr = restricted_mean_survival_time(km_rr, t=tau, return_variance=False)
 
                rules_dict["CR RMST"] = rmst_cr
                rules_dict["RR RMST"] = rmst_rr
                rules_dict["ER RMST"] = rmst_er
 
                rules_dict["median_survival_time_CR"] = rule.conclusion.value
                rules_dict["median_survival_time_RR"] = reference_rule.conclusion.value
                rules_dict["median_survival_time_ER"] = exception_rule.conclusion.value
