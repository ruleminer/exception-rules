...
from exception_rules.regression.algorithm import ExceptionRulesRegressor as ExceptionRulesRegressor
...
class Experiment:
    ...
    def _get_models(self):
        ...
        algorithm = ExceptionRulesRegressor(mincov=5, induction_measure="c2", max_growing = 5, logger=self.logger, find_exceptions=True)
    ...
    def _draw_histograms(self, rule, i, path, X, y):    
        ...
        bins_y = int(np.sqrt(len(y)))
        bins_y_cr = int(np.sqrt(len(y_cr)))
        bins_y_rr = int(np.sqrt(len(y_rr)))
        bins_y_er = int(np.sqrt(len(y_er)))

        # Tworzenie histogramu
        plt.figure(figsize=(10, 6))
        plt.hist(y, bins=bins_y, alpha=0.5, label='y', color='blue', edgecolor='black')
        plt.hist(y_cr, bins=bins_y_cr, alpha=0.5, label='y_cr', color='green', edgecolor='black')
        plt.hist(y_rr, bins=bins_y_rr, alpha=0.5, label='y_rr', color='purple', edgecolor='black')
        plt.hist(y_er, bins=bins_y_er, alpha=1, label='y_er', color='red', edgecolor='black')
    ...
    def _evaluate_rules(self, ruleset, dataset, X, y):
        ...
        for rule in rules:
            if rule.exception_rule is not None: 
                ...
                stat, p_cr_rr = mannwhitneyu(y_cr, y_rr)
                stat, p_er_cr = mannwhitneyu(y_er, y_cr)
                stat, p_er_rr = mannwhitneyu(y_er, y_rr)
 
                rules_dict["CR_vs_RR_p_value"] = p_cr_rr
                rules_dict["ER_vs_CR_p_value"] = p_er_cr
                rules_dict["ER_vs_RR_p_value"] = p_er_rr
