...
from regression_tree_algorithm import DecisionTreeAlgorithm

from rulekit_algorithm_regression import RuleKitAlgorithm
...
class Experiment:
    ...
    def _get_models(self):
        ...
        return {
                ...
                "algorithm_rer": algorithm_rer,
                "algorithm_rer_with_exceptions": algorithm_rer_with_exceptions,
    ...
    def _get_results_for_algorithm(self, ruleset, model_name, results, X_train, y_train, X_test, y_test, dataset):
        ...
        train_predictions = ruleset.predict(X_train)
        test_predictions = ruleset.predict(X_test)

        results["rmse_train"] = root_mean_squared_error(y_train, train_predictions)
        results["rmse_test"] = root_mean_squared_error(y_test, test_predictions)
        results["mae_train"] = mean_absolute_error(y_train, train_predictions)
        results["mae_test"] = mean_absolute_error(y_test, test_predictions)
